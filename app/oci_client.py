        ...
        * **Oracle 建 VCN 时自带的默认安全列表**：里面有一条 ``TCP 22 from
        ...
        if at_risk and not force:
            return OperationResult(
                ok=False,
                message=(
                    "收紧后这 " + str(len(at_risk)) + " 台会失去入站（它们目前是靠子网的放行进来的）："
                    + chr(10)
                    + chr(10).join("  · " + x for x in at_risk)
                    + chr(10)
                    # 同一个子网里有好几台机器时，用户会连着撞上这条。光说「给它们加规则」
                    # 等于把他推回去研究 NSG 是什么 —— 而他要的恰恰是不用懂这个。
                    # 直接给出下一步:去那几台上点同一个按钮，做完再回来。
                    + "请先到这几台的防火墙页各点一次「一键修复防火墙」"
                    "（会把子网现在的放行搬到它们各自名下，可达性不变），最后一台修完时"
                    "子网这一步会自动完成。" + chr(10)
                    + "确实要带着「它们会连不上」这个后果继续，就选择仍然继续。"
                ),
